 ...
 */
public class TransactionalIndexedCollection2<O> extends ConcurrentIndexedCollection<O> {
    ...
    public TransactionalIndexedCollection2(Class<O> objectType) {
     ...
     */
    public <A extends Comparable<A>> TransactionalIndexedCollection2(Class<O> objectType, Persistence<O, A> persistence) {
        super(persistence);
    ...
    public ResultSet<O> retrieve(Query<O> query, QueryOptions queryOptions) {
        ...
        long thisVersionNumber = -1;
        Version thisVersion = null;
        while (thisVersion == null) { // need to loop as current version could go in between calls.
            thisVersionNumber = currentVersion;
            thisVersion = versions.get(thisVersionNumber);
        }
        ...
        Version finalThisVersion = thisVersion;
        long finalThisVersionNumber = thisVersionNumber;
        CloseableResultSet<O> versionReadingResultSet = new CloseableResultSet<O>(results, query, queryOptions) {
            ...
            public void close() {
                ...
                long decrementedReadersCountForThisVersion = finalThisVersion.readersCount.decrementAndGet();
                if (decrementedReadersCountForThisVersion == 0) {
                    ...
                    if (finalThisVersionNumber != currentVersion) {
                        ...
                        finalThisVersion.writeLock.release();
                    }
                }
            }
        };
        ...
        if (thisVersion.objectsToExclude.iterator().hasNext()) {
            ...
            return new CloseableFilteringResultSet<O>(versionReadingResultSet, query, queryOptions) {
                ...
                public boolean isValid(O object, QueryOptions queryOptions) {
                    @SuppressWarnings("unchecked")
                    Iterable<O> objectsToExclude = (Iterable<O>)finalThisVersion.objectsToExclude;
